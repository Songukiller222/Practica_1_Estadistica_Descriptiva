...
FastFoodNutritionMenuV2$Weight.Watchers.Pnts <- as.numeric(FastFoodNutritionMenuV2$Weight.Watchers.Pnts)
...
tabla_frecuencias_Calories <- tabla_frecuencias_Calories |> mutate(clase = cut(Calories, breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))

tabla_frecuencias_Calories <- tabla_frecuencias_Calories |>
  arrange(clase) |>  kable(digits = 4) 
...
tabla_frecuencias_Fat <- tabla_frecuencias_Fat |> mutate(clase = cut(Total.Fat..g., breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))
...
tabla_frecuencias_Cholesterol <- tabla_frecuencias_Cholesterol |> mutate(clase = cut(Cholesterol..mg., breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))
...
tabla_frecuencias_Sodium <- tabla_frecuencias_Sodium |> mutate(clase = cut(Sodium...mg., breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))
...
tabla_frecuencias_Carbs <- tabla_frecuencias_Carbs |> mutate(clase = cut(Carbs..g., breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))
...
tabla_frecuencias_Fiber <- tabla_frecuencias_Fiber |> mutate(clase = cut(Fiber..g., breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))
...
tabla_frecuencias_Sugars <- tabla_frecuencias_Sugars |> mutate(clase = cut(Sugars..g., breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))
...
tabla_frecuencias_Protein <- tabla_frecuencias_Protein |> mutate(clase = cut(Protein..g., breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))
...
#### Weight
```{r}
histo <- hist(FastFoodNutritionMenuV2$Weight.Watchers.Pnts, plot = FALSE)

tabla_frecuencias_Weight <- FastFoodNutritionMenuV2 |> count(Weight.Watchers.Pnts) |> mutate(f = n/nrow(FastFoodNutritionMenuV2), N= cumsum(n), F = cumsum(f))
  
tabla_frecuencias_Weight <- tabla_frecuencias_Weight |> mutate(clase = cut(Weight.Watchers.Pnts, breaks = histo$breaks)) |>
group_by(clase) |> summarise(n = sum(n), f = sum(f), N = sum(n),  F = sum(f))

tabla_frecuencias_Weight <- tabla_frecuencias_Weight |>
  arrange(clase) |>  kable(digits = 4) 
print(tabla_frecuencias_Weight)
