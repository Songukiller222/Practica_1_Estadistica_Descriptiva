---
title: "Práctica sobre la nutrición"
...
En primer lugar, se precisa de la instalación de diversas librerías de "R" para llevar a cabo el análisis descriptivo.
...
El histograma muestra que la mayoría de los productos tienen alrededor de 225 kcal, lo que concuerda con lo obtenido en la tabla por intervalos. 
...
La mayoría de productos tienen alrededor de 13 gramos de grasas. Además se vuelve a mostrar gran cantidad de valores nulos. Además, se ve que al aumentar la grasa, hay menos productos, por lo que es una relación inversamente proporcional.
...
FastFoodNutritionMenuV2 <- FastFoodNutritionMenuV2 |>
  mutate(Intervalo_de_grasas_saturadas = cut(Saturated.Fat..g., breaks = histo$breaks))

tabla_frecuencias_Saturated <- FastFoodNutritionMenuV2 |>
  group_by(Intervalo_de_grasas_saturadas) |>
  summarise(n = n(), f = n() / nrow(FastFoodNutritionMenuV2)) |>
  mutate(N = cumsum(n), F = cumsum(f))

tabla_frecuencias_Saturated <- tabla_frecuencias_Saturated |>
  arrange(Intervalo_de_grasas_saturadas) |>
  kable(digits = 4)
...
FastFoodNutritionMenuV2 |> ggplot(aes(Saturated.Fat..g.)) +
  geom_histogram(fill = "cyan", bins = 15, col = "white") + labs(title = "Histograma de las grasas saturadas",
       x = "Intervalos de grasas saturadas", y = "Frecuencia") +
  theme_bw()
```

La mayoría de productos tienen alrededor de 4 gramos de grasas saturadas. También se observa cómo a medida que se aumenta los gramos, disminuye la cantidad de productos que se encuentran. Siendo una relación inversamente rpoporcional. 
...
FastFoodNutritionMenuV2 |> ggplot(aes(Cholesterol..mg.)) +
  geom_histogram(fill = "cyan", bins = 15, col = "white") + labs(title = "Histograma del colesterol",
       x = "Intervalos del colesterol", y = "Frecuencia") +
  theme_bw()
```

La mayoría de productos tienen alrededor de 50 mg de colesterol. Hay algúnos productos que se acercan a 600 mg, lo que supone una cantidad excesiva de colesterol en un solo producto. Por tanto, esos productos son muy dañinos para la salud.
...
FastFoodNutritionMenuV2 |> ggplot(aes(Sodium...mg.)) +
  geom_histogram(fill = "cyan", bins = 15, col = "white") + labs(title = "Histograma de sodio",
       x = "Intervalos de sodio", y = "Frecuencia") +
  theme_bw()
```

La mayoría de productos tienen alrededor de 300 mg de sodio, y la cantidad de productos disminuye cuando se aumenta los mg de sodio. Por lo que es una relación inversamente proporcional.
...
FastFoodNutritionMenuV2 |> ggplot(aes(Carbs..g.)) +
  geom_histogram(fill = "cyan", bins = 15, col = "white") + labs(title = "Histograma de carbohidratos",
       x = "Intervalos de carbohidratos", y = "Frecuencia") +
  theme_bw()
```
...
FastFoodNutritionMenuV2 |> ggplot(aes(Fiber..g.)) +
  geom_histogram(fill = "cyan", bins = 15, col = "white") + labs(title = "Histograma de fibra",
       x = "Intervalos de fibra", y = "Frecuencia") +
  theme_bw()
```

Si no se tiene en cuenta los valores nulos, lo más común son 3 gramos de fibra. Además, el histograma presenta una relación inversamente proporcional.
...
FastFoodNutritionMenuV2 |> ggplot(aes(Sugars..g.)) +
  geom_histogram(fill = "cyan", bins = 15, col = "white") + labs(title = "Histograma del azúcar",
       x = "Intervalos del azúcar", y = "Frecuencia") +
  theme_bw()
```

Unos 30 gramos de azúcar es lo que más se repite en el histograma, lo que supondría aproximadamente entre un 50% y un 60% del azúcar total recomendable al día. Por lo que la mayoría de los productos contienen una cantidad de azúcar muy grande.
...
FastFoodNutritionMenuV2 |> ggplot(aes(Protein..g.)) +
  geom_histogram(fill = "cyan", bins = 15, col = "white") + labs(title = "Histograma de la proteína",
       x = "Intervalos de la proteína", y = "Frecuencia") +
  theme_bw()
```
...
Para acabar con el estudio univariante, se hará un resumen numérico comparando los valores de todas las variables. Así se podrán obtener otros valores como la moda, el máximo, el mínimo, los cuartiles, etc.
...
Se han selecionado cuatro variables por ser las más importantes nutricionalmente. Estas son Calorías, Proteína, Grasas_Totales, Colesterol.
...
Debido a que los valores de la covarianza no están normalizados, no se puede apreciar la información adecuadamente. De este modo se procederá a hacer el gráfico de la correlación para facilitar la observación de las relaciones lineales:
...
Como se puede observar, la correlación entre todos los valores es una relación lineal positiva. Siendo Colesterol-Calorías las variables que tienen menos relación lineal y GrasaTotal-Calorías las variables con más relación lineal.
...
```{r}
FastFoodNutritionMenuV2 |> 
  ggplot(aes(x=Company, y=Calories))+geom_boxplot()+theme_bw()+labs(title= "Gráfico de cajas (Compañía-Calorías)", y = "Calorías (kcal)",x = "Compañía")
...
```{r}
FastFoodNutritionMenuV2 |> 
  ggplot(aes(x = Protein..g., fill = Company)) + geom_density(alpha = 0.5)+labs(title = "Gráfico de densidad de proteínas", 
       x = "Proteína (g)",
       y = "Densidad")+theme_bw()
```
...
```{r}
FastFoodNutritionMenuV2 |> 
  ggplot(aes(x=Company, y=Protein..g.))+geom_boxplot()+theme_bw()+labs(title="Gráfico de cajas (Compañía-Proteína)", y = "Proteína (g)",x = "Compañía")

```


Como se ha visto en el gráfico anterior, KFC presenta una mediana que no se corresponde a la realidad debido a que hay demasiados valores nulos. Burger King y Wendy's no tienen primer cuartil, lo que es un indicador de que los datos pueden ser erróneos o nulos.
...
```{r}
FastFoodNutritionMenuV2 |>
  ggplot(aes(x=Company, y=Total.Fat..g.))+geom_boxplot()+theme_bw()+labs(title="Gráfico de cajas (Compañía-Grasas)",y = "Grasas (g)",x = "Compañía")
```
...
Mc Donald's carece de primer cuartil, por lo que puede haber datos erróneos.
...
Con este gráfico se puede ver como la mayoría de productos tienen su colesterol en menos de 100, pero que hay varios productos de algunas compañías que tienen bastante más.
...
```{r}
FastFoodNutritionMenuV2 |> 
  ggplot(aes(x=Company, y=Cholesterol..mg.))+geom_boxplot()+theme_bw()+labs(title="Gráfico de cajas (Compañía-Colesterol)", y = "Colesterol (mg)",x = "Compañía")
```

Con este gráfico, se confirma que en la variable Colesterol se encuentran muchos casos atípicos para todas las compañías a excepción de Pizza Hut, que no tiene ninguno. Algunos valores pueden llegan a ser 500 veces mayores que la mediana de su compañía, lo que es preocupante para la salud.
...
Para terminar de relacionar las variables, se procederá a hacer un resumen de lo anterior, midiendo la proporción de la varianza con r-cuadrado.
...
Al analizar los estos gráficos con ajuste de regresión de las variables grasas y colesterol, a primera vista se puede observar como en el primero no se obtiene una línea tan recta como en el caso anterior, por lo que se podría pensar que no están tan positivamente relacionado. En cambio, si se fija uno más detenidamente, se ve cómo al principio, dónde se encuentran la mayoría de los productos, sí sigue una línea bastante recta, muy similar a la encontrada en el segundo gráfico, por lo que se puede decir que al comienzo sí que sigue una relación lineal positiva.

Sin embargo, cuanto más aumenta la grasa, este positivismo se va perdiendo y el ajuste de regresión va dirigiéndose hacia abajo, es decir, a más grasa, menos colesterol, comenzando a se una relación inversa. En parte, este hecho se debe a que quedan pocos productos con tan altos valores, por lo que uno solo puede hacer que este ajuste se desvíe tanto.
...
A continuación, se analizarán estas dos variables en función de la compañía en la que se encuentra cada producto. En él se puede observar cómo Pizza Hut vuelve a tener una relación muy positiva, además de que posee todos sus productos en el primer intervalo. En el resto de compañías se puede apreciar cómo sus valores están mucho más dispersos, no siguiendo ningún tipo de patrón, por lo que no se puede ver tan claro como es su relación, aunque sí se podría decir que en su mayoría siguen una relación positiva. También se observa en este gráfico cómo McDonald’s es la compañía cuyos productos tienen más colesterol, llegando a alcanzar casi los 600 miligramos. Así mismo, KFC posee el producto más grasiento consiguiendo casi los 100 gramos de grasas.

Por último, se relacionarán estas variables, haciendo un resumen de lo anterior, midiendo la proporción de la varianza con r-cuadrado.
